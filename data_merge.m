...
xlswrite('./Sensor_Location/Sen_XY.xlsx', Sen_XY, 'Sheet1');
...
common_index = [3,2];
diff_index_num = 3;  %which means that the train set contains length(common_index) + 3 sensors
sub_train_num = 20;
% Time series reconstruction training data preparation
SubTrainData(train_counts_data,SEQ_ID_TABLE,sub_train_num,...
...
SubTestData(test_counts_data,SEQ_ID_TABLE,common_index,test_boundry,TIME_SLOT,SEN_NUM);
% [EMB_ROW, EMB_COL] = size(embd_loc);
% [DAT_ROW, DAT_COL] = size(selected_date);
% SEN_NUM = 3;
% %SEN_VEC = expect_index();
% 
% total_counts_data = zeros(SEN_NUM*DAT_ROW,(DAT_COL+EMB_COL+1));
